...
if (!function_exists('show_owned_category_list')) {
    function show_owned_category_list($dropdown = false, $intro = true)
    {
        ...
        if ($dropdown) { // show dropdown
            ...
            <form method="get" action="<?= $page ?>" class="show_owned_select_form">
                <p>
                    <?php
...
if (isset($_GET)) {
    foreach ($_GET as $get => $value) {
        if ($get != 'cat') {
            $page .= $connector . StringUtils::instance()->clean($get) . '=' . StringUtils::instance()->clean($value);
            $connector = '&amp;';
        }
    }
}
...
if ((isset($_GET['cat']) && $_GET['cat'] != '') ||
    ...
    if ((isset($_GET['cat']) && $_GET['cat'] == 'all') ||
        ...
        if (isset($_GET['cat']) && ctype_digit($_GET['cat'])) {
            $catid = StringUtils::instance()->clean($_GET['cat']);
        } else if (isset($show)) {
            $catid = $show;
        }
    ...
    if ((!isset($show_subcats_in_main_list) || !$show_subcats_in_main_list)
        ...
        $children = get_enth_category_children(StringUtils::instance()->clean($_GET['cat']));
        $cats = [];

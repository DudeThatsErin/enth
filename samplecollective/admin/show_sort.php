...
$set = 0;
$selected = []; // what have we selected?
...
foreach ($selected as $field => $value) {
    $field = RobotessNet\StringUtils::instance()->clean($field);
    $value = RobotessNet\StringUtils::instance()->clean($value);
...
if ($info['dropdown'] == 1) {
    ...
    <script type="text/javascript">
        ...
        function change(form) {
            ...
            if (dropDown.options[myIndex].value != "0") {
                window.open("<?= $list_url . $connector . $sortfield ?>=" +
    ...
    <form method="get" action="<?= $list_url ?>" class="show_sort_form">
        <p>
            <select name="<?= $sortfield ?>" onchange="change( this.form );">
                <option value="0"> Select sort option</option>
                <option value="all"> All <?= $member_type ?></option>
                <?php
